...
Large legacy C++ codebases built with CMake often need to consume a Rust
crate (compiled as a `staticlib` or `cdylib`) plus its C header (usually
generated by `cbindgen`). That wiring is normally done by hand: locating
`target/release/libfoo.a`, finding the header, and writing a bespoke
`Find*.cmake` file per crate. `cargo-cmake-bridge` automates that by reading
`cargo metadata`, locating the built artifact, and emitting a standard
CMake find-module plus an `IMPORTED` target you can `target_link_libraries`
against.
...
```
Cargo-Maker/
├── Cargo.toml                     # binary crate manifest (bin name: cargo-cmake-bridge)
├── README.md                      # this file
...
└── tests/
    └── generator_tests.rs         # end-to-end integration test
```
